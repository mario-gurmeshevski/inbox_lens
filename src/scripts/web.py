...
def _on_update_available(result: dict) -> None:
    """Background-checker callback: push an SSE event so the banner can appear live."""
    try:
...
def _update_info(db_path: str = DB_PATH) -> dict:
    """Update-panel context shared by the settings page and API status endpoint."""
...
# --------------------------------------------------------------------------- #
# In-app update: banner + settings panel + actions
# --------------------------------------------------------------------------- #
