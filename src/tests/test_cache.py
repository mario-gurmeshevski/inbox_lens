...
class TestReconcileInbox:
    ...
    def test_skips_when_fetch_ratio_below_threshold(self, tmp_db):
        ...
        removed = cache.reconcile_inbox(
            tmp_db, set(), protected_hashes=set(), searched_count=100
        )
        assert removed == 0
    ...
    def test_skips_when_ghosts_exceed_cap(self, tmp_db):
        ...
        removed = cache.reconcile_inbox(
            tmp_db, {one_hash}, protected_hashes=set(), searched_count=20
        )
        assert removed == 0
        # All rows preserved.
    ...
    def test_no_changes_when_everything_present(self, tmp_db):
        ...
        removed = cache.reconcile_inbox(
            tmp_db, {cache._hash_message_id("<a@e.com>")}, protected_hashes=set()
        )
        assert removed == 0
...
class TestSchemaMigration:
    ...
    def test_migrate_backfills_null_thread_id(self, tmp_path):
        ...
        with sqlite3.connect(db_path) as conn:
            conn.executescript(old_schema)
            conn.execute(
                "INSERT INTO emails (message_id_hash, message_id, thread_id) "
                "VALUES ('h1', '<m1@e.com>', NULL)"
            )
            conn.execute(
                "INSERT INTO emails (message_id_hash, message_id, thread_id) "
                "VALUES ('h2', '<m2@e.com>', NULL)"
            )
            conn.execute(
                "INSERT INTO emails (message_id_hash, message_id, thread_id) "
                "VALUES ('h3', '<m3@e.com>', 'keep-me')"
            )
